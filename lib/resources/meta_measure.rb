...
def run_measure(model, measure, argument_map, runner)
    ...
    begin
      ...
      result_child.stepValues.each do |arg_child|
        skip = false
        if arg_child.name.include? "hvac_cooling_capacity_w"
          runner.result.stepValues.each do |arg|
            if arg.name.to_s.include? "hvac_heating_capacity_w"
              skip = true
            end
          end
        elsif arg_child.name.include? "hvac_heating_capacity_w"
          runner.result.stepValues.each do |arg|
            if arg.name.to_s.include? "hvac_heating_capacity_w"
              skip = true
            end
        ...
        next if skip
        runner.registerValue(arg_child.name, arg_child.valueAsVariant.to_s)
      end
